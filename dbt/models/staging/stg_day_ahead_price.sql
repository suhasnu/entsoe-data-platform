{{ config(materialized='view') }}

with latest as (

    select
        zone_code,
        ts_utc,
        price_eur_mwh,
        resolution_minutes,
        ingested_at,
        row_number() over (
            partition by zone_code, ts_utc
            order by ingested_at desc
        ) as revision_rank
    from {{ source('bronze', 'raw_day_ahead_price') }}
    where ts_utc >= timestamp('2020-01-01')

)

-- An hourly average of four quarter-hourly clearing prices is a derived figure,
-- not a price anyone traded at. intervals_aggregated says which rows are derived.
select
    zone_code,
    timestamp_trunc(ts_utc, hour) as ts_hour_utc,
    avg(price_eur_mwh) as price_eur_mwh,
    count(*) as intervals_aggregated,
    max(resolution_minutes) as source_resolution_minutes,
    max(ingested_at) as ingested_at
from latest
where revision_rank = 1
  and price_eur_mwh is not null
group by zone_code, ts_hour_utc